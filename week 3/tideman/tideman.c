...
void record_preferences(int ranks[])
{
    ...
    for (int a = 0; a < candidate_count - 1; a++)
    {
        ...
        for (int b = a + 1; b < candidate_count; b++)
        {
            ...
            preferences[firstRank][secondRank]++;
    ...
    else
    {
        ...
        for (int i = 0; i < candidate_count; i++)
        {
            ...
            if (locked[loser][i])
            {
                ...
                if (cannot_lock(winner, newLoser))
                {
                    return true;
                }
            }
...
// TRY IF YOU SEE THIS AGAIN, BUT I FAILED USING A LOOP INSTEAD OF RECURSIVE FUNCTION
// {
//     // TODO
//     for (int a = 0; a < pair_count; a++)
//     {
...
// taken from youtube but it seems wrong
//
// bool has_cycle_helper(int index, bool visited[])
// {
//     if (visited[index])
